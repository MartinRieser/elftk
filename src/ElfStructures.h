 ...
 *
 * This file defines all the fundamental data structures used in multi-architecture ELF binary analysis
 * supporting ARM, ARM64, RISC-V, and Xtensa processors:
 * 
...
struct Elf32_Ehdr {
    ...
    [[nodiscard]] std::string getArchitectureName() const noexcept {
        switch (e_machine) {
            case ElfConstants::ARM_MACHINE: return "ARM";
            case ElfConstants::AARCH64_MACHINE: return "ARM64";
            case ElfConstants::RISCV_MACHINE: return "RISC-V";
            case ElfConstants::XTENSA_MACHINE: return "Xtensa";
...
struct Elf32_Sym {
    ...
    [[nodiscard]] constexpr uint8_t getType() const noexcept { return st_info & 0xf; }

    /** @brief Extract symbol binding from st_info field (STB_LOCAL, STB_GLOBAL, etc.) */
    [[nodiscard]] constexpr uint8_t getBind() const noexcept { return st_info >> 4; }

    /** @brief Check if symbol is a function */
    [[nodiscard]] constexpr bool isFunction() const noexcept { return getType() == 2; } // STT_FUNC

    /** @brief Check if symbol is a data object */
    [[nodiscard]] constexpr bool isObject() const noexcept { return getType() == 1; } // STT_OBJECT

    /** @brief Check if symbol is globally visible */
    [[nodiscard]] constexpr bool isGlobal() const noexcept { return getBind() == 1; } // STB_GLOBAL
};
...
struct TypeInfo {
    ...
    [[nodiscard]] const TypeMember* findMember(const std::string& name) const noexcept {
        auto it = std::find_if(members.begin(), members.end(),
                              [&name](const TypeMember& member) { return member.name == name; });
