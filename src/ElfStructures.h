 ...
 *
 * This file defines all the fundamental data structures used in multi-architecture ELF binary
 * analysis supporting ARM, ARM64, RISC-V, and Xtensa processors:
 *
 ...
 * - **Multi-Architecture**: Supports ARM, ARM64, RISC-V, and Xtensa with 32-bit and 64-bit variants
 * - **Type Safety**: Strong typing with enums and validation methods to prevent invalid values
 ...
 * - Parse raw binary data from multi-architecture ELF files
 ...
 *
 * @note Structures handle both little-endian and big-endian systems automatically
...
// Include libdwarf first to avoid macro conflicts
#ifdef HAVE_LIBDWARF
    #include <dwarf.h>
    #include <libdwarf.h>
#else
...
// ============================================================================
// SECTION 1: ELF FORMAT ENUMERATIONS AND CONSTANTS
// ============================================================================
//
// This section contains all ELF-related enumerations and constants used for
// parsing ELF binary files across all supported architectures.
//
 ...
 * architectures across all supported processor families.
 *
 * ## Usage in Multi-Architecture Development:
 * - **ELFCLASS32**: ARM Cortex-M/A32/R, RISC-V RV32, Xtensa ESP32 (most embedded systems)
 * - **ELFCLASS64**: ARM64/AArch64, RISC-V RV64, x86_64 (application processors)
 *
 ...
 * Most modern embedded processors use little-endian encoding.
 *
 * ## Practical Impact:
 ...
 * - **ARM**: Modern cores default to little-endian (Cortex-M are little-endian only)
 * - **RISC-V**: Little-endian is standard, big-endian support varies
 * - **Xtensa**: Configurable, ESP32 family uses little-endian
 *
 ...
 * the machine types supported by elftk for embedded system analysis.
 *
 * ## Supported Architectures:
 * - **EM_ARM**: 32-bit ARM processors (Cortex-M/A/R series, classic ARM)
 * - **EM_AARCH64**: 64-bit ARM processors (ARM64/AArch64, Cortex-A53/57/72/78)
 * - **EM_RISCV**: RISC-V processors (both RV32 and RV64 variants)
 * - **EM_XTENSA**: Tensilica Xtensa processors (ESP32, ESP32-S2/S3/C3/C6/H2)
 *
 ...
 * - **ARM**: Separate machine types for 32-bit (EM_ARM) and 64-bit (EM_AARCH64)
 * - **RISC-V**: Single EM_RISCV value, use ELFCLASS32/64 to distinguish RV32/RV64
 * - **Xtensa**: Highly configurable architecture, ESP32 family most common
 */
enum class ElfMachine : uint16_t {
    ...
    EM_XTENSA = 0x5E    ///< Tensilica Xtensa processors (ESP32, ESP32-S2/S3/C3/C6/H2)
};

// ELF machine type constants for validation
namespace ElfConstants {
constexpr uint16_t ARM_MACHINE = 0x28;
constexpr uint16_t AARCH64_MACHINE = 0xB7;
constexpr uint16_t RISCV_MACHINE = 0xF3;
constexpr uint16_t XTENSA_MACHINE = 0x5E;

constexpr uint8_t ELF_CLASS_32 = 1;
constexpr uint8_t ELF_CLASS_64 = 2;

constexpr uint8_t ELF_DATA_LSB = 1;
constexpr uint8_t ELF_DATA_MSB = 2;
}  // namespace ElfConstants
 ...
 *
 * ## Multi-Architecture Symbol Analysis:
 * - **STT_FUNC**: Function symbols (ARM Thumb functions have LSB set in address)
 ...
 *
 * ## Architecture-Specific Notes:
 * - **ARM**: Thumb functions indicated by LSB set in function address
 * - **RISC-V**: Compressed instruction support affects function alignment
 * - **Xtensa**: Variable-length instructions, function addresses aligned
 *
...
// ============================================================================
// SECTION 2: DWARF DEBUG INFORMATION ENUMERATIONS
// ============================================================================
//
// This section contains DWARF-specific enumerations for parsing debug
// information from .debug_* sections in ELF files.
//
...
// ============================================================================
// SECTION 3: DWARF DEBUG INFORMATION ENTRY (DIE) STRUCTURES
// ============================================================================
//
// This section contains structures for representing DWARF Debug Information
// Entries (DIEs) and related data from the legacy DWARF parser.
//
 ...
 * - **has_children**: Whether this DIE has child DIEs
...
struct DwarfDIE {
     ...
     * - Architecture differences in type sizes are handled automatically
     */
    [[nodiscard]] uint32_t getByteSize() const noexcept {
     ...
     *
     * ## Multi-Architecture ABI Alignment Examples:
     ...
     * - ARM AAPCS, RISC-V, and Xtensa ABIs handle alignment automatically
     */
    [[nodiscard]] uint32_t getDataMemberLocation() const noexcept {
        auto it = attributes.find(DwarfAttribute::DW_AT_data_member_location);
        return (it != attributes.end()) ? std::get<uint32_t>(it->second) : 0;
    }

    /**
     * @brief Check if this DIE has a specific attribute
     * @param attr Attribute to check for
     * @return true if attribute exists, false otherwise
     */
    [[nodiscard]] bool hasAttribute(DwarfAttribute attr) const noexcept {
        return attributes.find(attr) != attributes.end();
    }

    /**
     * @brief Validate this DIE for common structural issues
     * @return true if DIE appears valid, false if structural problems detected
     */
    [[nodiscard]] bool isValid() const noexcept {
        // Basic validation - must have valid tag and abbrev code
        return tag != 0 && abbrev_code != 0;
    }
};
#else
// Legacy DWARF parsing is deprecated in favor of libdwarf-based parsing
struct DwarfDIE {
    uint32_t offset;       ///< Offset in DWARF section (unique identifier for this DIE)
    uint32_t abbrev_code;  ///< Abbreviation code referencing attribute definitions
    uint32_t tag;          ///< DWARF tag identifying element type
    bool has_children;     ///< True if this DIE has child DIEs
     ...
     * - std::string: Names, string constants
     */
    std::map<uint32_t, std::variant<uint32_t, uint64_t, std::string>> attributes;

    std::vector<DwarfDIE> children;  ///< Child DIEs (e.g., structure members, nested scopes)

    // Legacy parser methods that work with libdwarf attribute codes
    std::string getName(uint32_t attr_code = 0x03) const;             // DW_AT_name
    uint32_t getByteSize(uint32_t attr_code = 0x0b) const;            // DW_AT_byte_size
...
inline std::string DwarfDIE::getName(uint32_t attr_code) const {
    auto it = attributes.find(attr_code);
    return (it != attributes.end()) ? std::get<std::string>(it->second) : "";
}

inline uint32_t DwarfDIE::getByteSize(uint32_t attr_code) const {
    auto it = attributes.find(attr_code);
    return (it != attributes.end()) ? std::get<uint32_t>(it->second) : 0;
}

inline uint32_t DwarfDIE::getType(uint32_t attr_code) const {
    auto it = attributes.find(attr_code);
    return (it != attributes.end()) ? std::get<uint32_t>(it->second) : 0;
}

inline uint32_t DwarfDIE::getDataMemberLocation(uint32_t attr_code) const {
    auto it = attributes.find(attr_code);
    return (it != attributes.end()) ? std::get<uint32_t>(it->second) : 0;
}
...
// ============================================================================
// SECTION 4: ELF BINARY FORMAT STRUCTURES
// ============================================================================
//
// This section contains structures that map directly to ELF binary format
// layout for 32-bit and 64-bit architectures.
//
 ...
 *
 * ## Multi-Architecture Context:
 * - **e_machine**: Architecture identifier (EM_ARM=0x28, EM_RISCV=0xF3, EM_XTENSA=0x5E)
 * - **e_entry**: Program entry point (ARM Thumb mode indicated by LSB set)
...
struct Elf32_Ehdr {
    ...
    /**
     * @brief Validate ELF header for structural correctness
     * @return true if header appears valid, false otherwise
     */
    [[nodiscard]] bool isValid() const noexcept {
        // Basic validation checks
        return e_ehsize >= 52 &&      // Minimum 32-bit ELF header size
               e_version == 1 &&      // Current ELF version
               e_shentsize >= 40 &&   // Minimum 32-bit section header size
               e_shstrndx < e_shnum;  // String table index within bounds
    }

    /**
     * @brief Check if this is a supported architecture
     * @return true if architecture is supported by elftk
     */
    [[nodiscard]] bool isSupportedArchitecture() const noexcept {
        return e_machine == ElfConstants::ARM_MACHINE ||
               e_machine == ElfConstants::AARCH64_MACHINE ||
               e_machine == ElfConstants::RISCV_MACHINE ||
               e_machine == ElfConstants::XTENSA_MACHINE;
    }
     ...
     */
    [[nodiscard]] std::string getArchitectureName() const noexcept {
        switch (e_machine) {
            case ElfConstants::ARM_MACHINE:
                return "ARM";
            case ElfConstants::AARCH64_MACHINE:
                return "ARM64";
            case ElfConstants::RISCV_MACHINE:
                return "RISC-V";
            case ElfConstants::XTENSA_MACHINE:
                return "Xtensa";
            default:
                return "Unknown";
        }
...
struct Elf64_Ehdr {
    ...
    [[nodiscard]] bool isValid() const noexcept {
        // Basic validation checks
        return e_ehsize >= 64 &&      // Minimum 64-bit ELF header size
               e_version == 1 &&      // Current ELF version
               e_shentsize >= 64 &&   // Minimum 64-bit section header size
               e_shstrndx < e_shnum;  // String table index within bounds
    }
 ...
 *
 * ## Architecture-Specific Notes:
 * - **ARM**: Function addresses with LSB set indicate Thumb mode
 * - **RISC-V**: Function addresses aligned to instruction boundaries
 * - **Xtensa**: Variable-length instructions affect symbol alignment
...
struct Elf32_Sym {
    ...
    [[nodiscard]] constexpr uint8_t getBind() const noexcept {
        return st_info >> 4;
    }

    /** @brief Check if symbol is a function */
    [[nodiscard]] constexpr bool isFunction() const noexcept {
        return getType() == 2;
    }  // STT_FUNC

    /** @brief Check if symbol is a data object */
    [[nodiscard]] constexpr bool isObject() const noexcept {
        return getType() == 1;
    }  // STT_OBJECT

    /** @brief Check if symbol is globally visible */
...
// ============================================================================
// SECTION 5: TYPE SYSTEM AND ANALYSIS RESULT STRUCTURES
// ============================================================================
//
// This section contains high-level structures for representing analysis
// results, type information, and program element metadata.
//
...
struct TypeMember {
    ...
    TypeMember(const std::string_view n,
...
struct TypeInfo {
     ...
     * @return true if type has members, false if empty
     */
    [[nodiscard]] bool hasMembers() const noexcept {
        return !members.empty();
    }

    /**
     * @brief Get number of members in this type
     * @return Member count
     */
    ...
    /**
     * @brief Find member by name
     * @param name Member name to search for
     * @return Pointer to member if found, nullptr otherwise
     */
    [[nodiscard]] const TypeMember* findMember(const std::string& name) const noexcept {
        auto it = std::find_if(members.begin(), members.end(), [&name](const TypeMember& member) {
    ...
    /**
     * @brief Validate type information for consistency
     * @return true if type appears valid, false if inconsistencies detected
     */
    [[nodiscard]] bool isValid() const noexcept {
        // Basic validation - size should be reasonable and DWARF offset non-zero
        return size > 0 && size < 1000000 &&  // Reasonable size limits
               dwarf_offset > 0;              // Valid DWARF reference
    }
};

// ============================================================================
// SECTION 6: ADVANCED TYPE RESOLUTION (DWARF 5)
// ============================================================================
//
// This section contains advanced type resolution structures for DWARF 5
// features including template analysis and optimization hints.
//

/**
 * @brief Advanced Type Resolution Data Structures (DWARF 5)
 */
...
struct TemplateParameter {
    ...
    TemplateParameter(const std::string& n,
                      const std::string& t,
                      const std::string& v,
                      bool is_type)
...
struct MemberInfo {
    ...
    std::string section_name;    ///< ELF section name where symbol is defined (e.g., ".text",
                                 ///< ".data", ".rodata")
    ...
    MemberInfo(const std::string_view n, uint64_t a, const std::string_view t)
        : name(n), address(a), type(t), is_constant(false) {}

    MemberInfo(const std::string_view n,
               uint64_t a,
               const std::string_view t,
               bool constant,
    ...
    // Enhanced constructor with detailed metadata
    MemberInfo(const std::string_view n,
               uint64_t a,
               const std::string_view t,
               uint64_t size,
               ...
               bool bitfield_flag = false,
               uint32_t bit_off = 0,
               uint32_t bit_sz = 0,
               const std::string_view enum_vals = "")
        : name(n),
          address(a),
          type(t),
          is_constant(false),
          byte_size(size),
          alignment(align),
          is_typedef(typedef_flag),
          underlying_type(underlying),
          is_union(union_flag),
          is_bitfield(bitfield_flag),
          bit_offset(bit_off),
          bit_size(bit_sz),
...
// ============================================================================
// SECTION 7: LOCAL VARIABLE LOCATION ANALYSIS
// ============================================================================
//
// This section contains structures for analyzing local variable locations
// and function stack frames using DWARF location expressions.
//

/**
 * @brief Local Variable Location Analysis Data Structures
 */
...
struct VariableLocation {
    ...
    VariableLocation()
        : type(LocationType::UNKNOWN), offset_or_register(0), description("unknown") {}
...
struct LocalVariableInfo {
    ...
    LocalVariableInfo()
        : scope_start_pc(0), scope_end_pc(0), is_parameter(false), parameter_index(0) {}

    LocalVariableInfo(const std::string& n,
                      const std::string& t,
...
struct FunctionInfo {
    ...
    std::vector<LocalVariableInfo>
        parameters;  ///< Function parameters only (subset of local_variables)
    ...
    bool isPotentialInterruptHandler() const {
        ...
        return (name.find("_Handler") != std::string::npos ||
                name.find("IRQ") != std::string::npos || name.find("_ISR") != std::string::npos ||
...
// ============================================================================
// SECTION 8: INTERRUPT VECTOR TABLE STRUCTURES
// ============================================================================
//
// This section contains structures for analyzing interrupt vector tables
// and handler functions in embedded systems.
//
...
struct InterruptVectorEntry {
    ...
    InterruptVectorEntry(uint32_t num,
                         uint64_t addr,
...
struct InterruptVectorTable {
    ...
    std::map<uint64_t, std::string>
        handler_functions;  ///< Map of handler addresses to function names
...
// ============================================================================
// SECTION 9: MEMORY REGION MAPPING AND VALIDATION
// ============================================================================
//
// This section contains structures for mapping and validating memory
// regions, program headers, and embedded system memory layouts.
//
...
struct MemoryRegion {
    ...
    MemoryRegion()
        : virtual_address(0),
          ...
          segment_type(ElfSegmentType::PT_NULL),
          permissions(0),
          is_loadable(false),
          is_valid(true) {}
    ...
    std::string getPermissionString() const {
        std::string result = "---";
        if (hasPermission(ElfSegmentFlags::PF_R))
            result[0] = 'r';
        if (hasPermission(ElfSegmentFlags::PF_W))
            result[1] = 'w';
        if (hasPermission(ElfSegmentFlags::PF_X))
            result[2] = 'x';
        return result;
    }
    ...
    bool containsAddress(uint64_t address) const {
        return address >= virtual_address && address < (virtual_address + memory_size);
    }
...
// ============================================================================
// SECTION 10: STACK FRAME ANALYSIS (Extended for Embedded Debugging)
// ============================================================================
//
// This section contains comprehensive stack frame analysis structures
// for embedded debugging and professional development tools.
//
...
struct StackFrameEntry {
    ...
    int64_t
        stack_offset;  ///< Offset from frame pointer (negative = local vars, positive = parameters)
...
struct StackFrameInfo {
    ...
    StackFrameInfo()
        : function_start_address(0),
          function_end_address(0),
          total_frame_size(0),
...
struct StackFrameAnalysisResults {
    ...
    StackFrameAnalysisResults()
        : total_functions_analyzed(0),
          functions_with_frame_pointer(0),
          leaf_functions(0),
    ...
    std::string getStatisticsSummary() const {
        return "Stack Frame Analysis: " + std::to_string(total_functions_analyzed) +
               " functions, " + std::to_string(functions_with_frame_pointer) + " with FP, " +
