...
size_t StringInterner::getEstimatedMemoryUsage() const {
    ...
    size_t bucket_array_size =
        bucket_count * sizeof(void*);  // Pointer to first node in each bucket
...
StringInterner::MemoryStats StringInterner::getStats() const {
    ...
    stats.total_characters = std::accumulate(
        pool_.begin(), pool_.end(), size_t(0), [](size_t total, const std::string& str) {
            return total + str.length();
        });

    stats.estimated_bytes = getEstimatedMemoryUsage();
    stats.average_length =
        static_cast<double>(stats.total_characters) / static_cast<double>(stats.unique_strings);
